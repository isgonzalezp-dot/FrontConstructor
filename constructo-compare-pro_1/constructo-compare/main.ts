...
/**
 * Informacion de precio y disponibilidad de un producto en una tienda especifica.
 */
...
/**
 * Representa un material de construccion con sus precios en multiples tiendas.
 */
interface Producto {
  id:               string;
  nombre:           string;
...
/**
 * Item dentro del panel de cotizacion.
 * Combina el producto con la tienda seleccionada y la cantidad deseada.
 */
interface QuoteItem {
  producto:           Producto;
  tiendaSeleccionada: TiendaInfo;
  cantidad:           number;
}

/**
 * Proyecto de cotizacion que agrupa multiples items.
 */
interface Proyecto {
  id:             string;
  nombre:         string;
...
/**
 * Respuesta del servicio de valor de la UF.
 */
interface UFData {
  valor: number;
  fecha: string;
 ...
 */
const AppState = {
  productos:        [] as Producto[],   // Catalogo completo cargado desde la API
  filteredProductos:[] as Producto[],   // Subconjunto filtrado por busqueda y categoria
  quoteItems:       [] as QuoteItem[],  // Items actuales del panel de cotizacion
  proyectos:        [] as Proyecto[],   // Proyectos guardados por el usuario
  currentProyecto:  null as Proyecto | null,
...
const CONFIG = {
  ...
  USE_MOCK:     true,
  MOCK_DELAY:   600,
  TIENDAS:      ["Sodimac", "Easy", "Imperial"] as const,
  CATEGORIAS:   [
    "Todos",
    "Cementos y Morteros",
    "Revestimientos",
...
const FakeAPI = {
   ...
   */
  async getProducts(query?: string, categoria?: string): Promise<Producto[]> {
    await delay(CONFIG.MOCK_DELAY);
    const response = await fetch("data/products.json");
    let productos: Producto[] = await response.json();
   ...
   */
  async getUFValue(): Promise<UFData> {
    await delay(300);
    return {
      valor: 37842.5,
   ...
   */
  async saveProyecto(proyecto: Proyecto): Promise<{ success: boolean; id: string }> {
    await delay(400);
 ...
 */
function formatUF(valorCLP: number): string {
 ...
 */
function getBestPrice(producto: Producto): TiendaInfo {
  const disponibles = producto.tiendas.filter((t) => t.stock);
  if (disponibles.length === 0) return producto.tiendas[0];
 ...
 */
function addToQuote(producto: Producto, tienda: TiendaInfo, cantidad = 1): void {
  const existing = AppState.quoteItems.find(
    (item) =>
 ...
 */
function removeFromQuote(index: number): void {
 ...
 */
function calculateTotal(): { clp: number; uf: string } {
  const total = AppState.quoteItems.reduce(
    (sum, item) => sum + item.tiendaSeleccionada.precio * item.cantidad,
    0
  );
 ...
 */
function calculateOptimizedTotal(): { clp: number; uf: string; ahorro: number } {
  const originalTotal = AppState.quoteItems.reduce(
    (sum, item) => sum + item.tiendaSeleccionada.precio * item.cantidad,
    0
  );
...
function renderProducts(productos: Producto[]): void {
  ...
  grid.querySelectorAll(".tienda-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (tiendaInfo.stock) addToQuote(producto, tiendaInfo);
    });
  });
}
 ...
 */
function renderProductCard(producto: Producto): string {
  const best     = getBestPrice(producto);
...
function updateQuoteButton(): void {
  ...
  const badge = btn.querySelector(".quote-badge") as HTMLElement;
...
async function init(): Promise<void> {
  ...
  if (ufEl) ufEl.textContent = `${formatCLP(ufData.valor)}`;
  ...
  document.getElementById("save-quote")?.addEventListener("click", () => {
    ...
    const authModule = (window as unknown as Record<string, unknown>).Auth as {
      isLoggedIn?: () => boolean;
      saveQuote?:  (q: unknown) => boolean;
    } | undefined;
...
async function loadProducts(): Promise<void> {
  ...
  try {
    const productos = await FakeAPI.getProducts(
...
function renderCategoryFilters(): void {
  ...
  container.innerHTML = CONFIG.CATEGORIAS.map(
    (cat) => `
  ...
  container.querySelectorAll(".filter-chip").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      AppState.selectedCategory = (btn as HTMLElement).dataset.cat!;
      await loadProducts();
    });
  });
}
 ...
 */
(window as unknown as Record<string, unknown>).updateQuantity  = updateQuantity;
(window as unknown as Record<string, unknown>).removeFromQuote = removeFromQuote;
